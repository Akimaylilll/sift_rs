...
impl SiftDescriptor {
    ...
    pub fn compute_descriptors(
        ...
        let mut gradient_magnitudes_pyramid: Vec<
            ndarray::ArrayBase<ndarray::OwnedRepr<f32>, ndarray::Dim<[usize; 3]>>,
        > = Vec::new();
        let mut gradient_orientations_pyramid: Vec<
            ndarray::ArrayBase<ndarray::OwnedRepr<f32>, ndarray::Dim<[usize; 3]>>,
        > = Vec::new();

        for (_octave, scale_space) in scale_spaces.iter().enumerate() {
            let (num_scales, height, width) = scale_space.dim();
            let mut magnitudes_pyramid =
                Array3::<f32>::zeros((num_scales, height as usize, width as usize));
            let mut orientations_pyramid =
                Array3::<f32>::zeros((num_scales, height as usize, width as usize));
        ...
        for keypoint in keypoints.drain(..) {
            ...
            let orientations =
                self.compute_keypoint_orientations(&keypoint, magnitude_layer, orientation_layer);
        ...
        for keypoint in all_keypoints.iter_mut() {
            ...
            keypoint.descriptor =
                self.compute_descriptor(keypoint, magnitude_layer, orientation_layer);
        }
    ...
    fn compute_keypoint_orientations(
        ...
        for dy in -radius..=radius {
            for dx in -radius..=radius {
                let sample_x = (kp.x / (1 << kp.octave) as f32 + dx as f32).round() as i32;
                let sample_y = (kp.y / (1 << kp.octave) as f32 + dy as f32).round() as i32;
            ...
            if curr > prev && curr > next && curr >= threshold {
                ...
                if denominator.abs() > 1e-6 {
                    ...
                    let interpolated_bin =
                        (interpolated_bin + self.num_bins as f32) % self.num_bins as f32;
                    let angle_rad =
                        interpolated_bin * 2.0 * std::f32::consts::PI / self.num_bins as f32;
    ...
    fn compute_descriptor(
        ...
        // 描述符向量初始化
        let mut descriptor_vector =
            vec![0.0; self.descriptor_width * self.descriptor_width * self.descriptor_num_bins];

        // 计算16x16邻域的大小参数（根据关键点尺度缩放）
        let descriptor_size = 16.0 * kp.scale;
        let cell_size = descriptor_size / self.descriptor_width as f32; // 4.0 * kp.scale
        ...
        // 在16x16窗口内采样
        let half_size = (descriptor_size / 2.0).ceil() as i32;
        for dy in -half_size..half_size {
            for dx in -half_size..half_size {
                ...
                // 计算在原图上的坐标
                let x = kp.x / (1 << kp.octave) as f32 + tr_x;
                let y = kp.y / (1 << kp.octave) as f32 + tr_y;
                ...
                // 将梯度方向映射到bin
                let ori_bin =
                    angle_diff * self.descriptor_num_bins as f32 / (2.0 * std::f32::consts::PI);
                ...
                if cx < -1.0
                    || cx >= self.descriptor_width as f32
                    || cy < -1.0
                    || cy >= self.descriptor_width as f32
                {
                    continue;
                }
                ...
                for (rr, wr) in [(r0, 1.0 - dr), (r0 + 1, dr)] {
                    if rr < 0 || rr >= self.descriptor_width as i32 {
                        continue;
                    }
                    for (cc, wc) in [(c0, 1.0 - dc), (c0 + 1, dc)] {
                        if cc < 0 || cc >= self.descriptor_width as i32 {
                            continue;
                        }
                        for (oo, wo) in [
                            (o0, 1.0 - do_),
                            ((o0 + 1) % self.descriptor_num_bins as i32, do_),
                        ] {
                            let rridx = rr as usize;
                            let ccidx = cc as usize;
                            let ooidx = (oo + self.descriptor_num_bins as i32)
                                % self.descriptor_num_bins as i32;

                            if rridx < self.descriptor_width
                                && ccidx < self.descriptor_width
                                && ooidx >= 0
                                && ooidx < self.descriptor_num_bins as i32
                            {
                                let idx = (rridx * self.descriptor_width + ccidx)
                                    * self.descriptor_num_bins
                                    + ooidx as usize;
